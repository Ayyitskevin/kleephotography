...
def test_apply_to_gallery_matches_assets_by_stored_name(tmp_path, monkeypatch):
    ...
    hero = db.one("SELECT argus_hero_asset_ids, argus_analyzed_count FROM galleries WHERE id=?", (gid,))
    assert hero["argus_analyzed_count"] == 2
    assert json.loads(hero["argus_hero_asset_ids"]) == [a1]

    row = db.one("SELECT argus_alt_text, argus_keywords, argus_keeper_score FROM assets WHERE id=?", (a1,))
