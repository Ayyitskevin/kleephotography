...
def get_form(form_id: int) -> "db.sqlite3.Row":
    f = db.one("SELECT * FROM forms WHERE id=?", (form_id,))
    if not f:
...
def get_field(field_id: int) -> "db.sqlite3.Row":
    fld = db.one("SELECT * FROM form_fields WHERE id=?", (field_id,))
    if not fld:
...
async def form_create(title: str = Form(...), kind: str = Form("lead")):
    ...
    fid = db.run("INSERT INTO forms (slug, title, kind) VALUES (?,?,?)",
                 (slug, title, kind))
...
async def form_builder(request: Request, form_id: int):
    ...
    subs = db.all_(
        """SELECT id, name, email, created_at FROM form_submissions
           WHERE form_id=? ORDER BY created_at DESC LIMIT 10""", (form_id,))
    n_subs = db.one("SELECT COUNT(*) AS n FROM form_submissions WHERE form_id=?",
                    (form_id,))["n"]
    parsed = []
    for fld in fields:
        d = dict(fld)
        ...
        parsed.append(d)
    return templates.TemplateResponse(request, "admin/form_builder.html",
                                      {"f": f, "fields": parsed, "subs": subs,
                                       "n_subs": n_subs, "ftypes": FTYPES,
                                       "ftype_labels": FTYPE_LABELS})


@router.post("/{form_id}")
async def form_update(form_id: int, title: str = Form(...), intro: str = Form(""),
                      active: bool = Form(False)):
    get_form(form_id)
    title = title.strip()
    if not title:
        raise HTTPException(status_code=400, detail="title required")
    db.run("UPDATE forms SET title=?, intro=?, active=? WHERE id=?",
...
async def field_add(form_id: int, label: str = Form(...), ftype: str = Form("short_text"),
                    required: bool = Form(False), options: str = Form("")):
    get_form(form_id)
    label = label.strip()
    if not label:
...
async def field_update(field_id: int, label: str = Form(...),
                       ftype: str = Form("short_text"), required: bool = Form(False),
                       options: str = Form("")):
...
async def field_move(field_id: int, dir: str = Form(...)):
    ...
    if neighbor:
        db.run("UPDATE form_fields SET sort_order=? WHERE id=?",
               (neighbor["sort_order"], fld["id"]))
        db.run("UPDATE form_fields SET sort_order=? WHERE id=?",
               (fld["sort_order"], neighbor["id"]))
    return RedirectResponse(f"/admin/forms/{fld['form_id']}", status_code=303)
...
async def form_submissions(request: Request, form_id: int):
    f = get_form(form_id)
    fields = db.all_(
        "SELECT id, label FROM form_fields WHERE form_id=? ORDER BY sort_order, id",
        (form_id,))
    rows = db.all_(
        """SELECT * FROM form_submissions WHERE form_id=?
           ORDER BY created_at DESC""", (form_id,))
    subs = []
    for r in rows:
        d = dict(r)
        try:
            ...
            d["answers"] = {}
        subs.append(d)
    return templates.TemplateResponse(request, "admin/form_submissions.html",
                                      {"f": f, "fields": fields, "subs": subs})
