...
def _integrations() -> list[dict]:
    ...
    notion_on = features.notion_enabled()
    notion_desc = "Mirror booking & session status to your workspace (one-way)"
    if notion_on:
        armed = [
            n
            for n, v in (
                ("Sessions", config.NOTION_SESSIONS_DB),
                ("Bookings", config.NOTION_BOOKINGS_DB),
            )
            if v
        ]
        notion_desc = "One-way mirror · " + (
            ", ".join(armed) + " armed" if armed else "token set, no database armed yet"
        )
    ...
    return [
        {
            ...
            **_badge(features.stripe_enabled()),
        },
        {
            "name": "Gmail",
            "mark": "M",
        ...
        },
        {
            ...
            **_badge(features.shots_api_enabled()),
        },
        {
...
def _operations() -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "value": f"after {config.CONTRACT_NUDGE_DAYS} days",
            "desc": "Telegram heads-up to you (not the client) when a sent contract is still unsigned — once per contract, outbound only",
        },
        {
...
def _storage() -> dict:
    ...
    return {
        ...
        "low": free_gb < config.MIN_FREE_GB,
        "backup": backup,
    }
