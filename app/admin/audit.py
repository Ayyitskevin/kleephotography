...
def _counts() -> dict:
    ...
    raw = db.all_(
        "SELECT entity_type FROM audit_log ORDER BY created_at DESC, id DESC LIMIT ?", (_LIMIT,)
    )
...
async def audit_log_view(request: Request, cat: str = "all"):
    ...
    filters = [
        {
            "key": k,
            "label": "All" if k == "all" else _CAT_META[k]["label"],
            "n": counts[k],
            "active": k == cat,
        }
        for k in _FILTERS
...
async def audit_csv():
    ...
    return PlainTextResponse(
        buf.getvalue(),
        headers={"Content-Disposition": 'attachment; filename="kleephotography_audit_log.csv"'},
    )
