...
async def dashboard(request: Request):
    ...
    totals = {
        ...
        "size": common.fmt_size(sum(sizes_b.values())),
    }
...
async def create_gallery(title: str = Form(...), client_name: str = Form("")):
    ...
    for i, name in enumerate(DEFAULT_SECTIONS):
        db.run("INSERT INTO sections (gallery_id, name, position) VALUES (?,?,?)", (gid, name, i))
...
def _transfer_card(g, size: str, today_iso: str, soon_iso: str) -> dict:
    ...
    if expired:
        when = f"link expired {common.short_date(exp)}"
    elif exp:
...
async def transfers(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        {"transfers": cards, "base_url": config.BASE_URL, "totals": totals},
    )
...
async def create_transfer(
    ...
    gid = db.run(
        """INSERT INTO galleries (slug, title, pin, type, require_pin,
                    published, expires_at) VALUES (?,?,?,'drop',?,1,?)""",
        (
            security.new_slug(),
            title.strip(),
            security.new_pin(),
            1 if require_pin else 0,
            expires_at,
        ),
    )
...
async def gallery_detail(request: Request, gallery_id: int):
    ...
    section_picks = {
        ...
        for r in db.all_(
           ...
           WHERE a.gallery_id=? GROUP BY a.section_id""",
            (gallery_id,),
        )
    }
    assets = db.all_(
    ...
    client = (
        db.one("SELECT name, email FROM clients WHERE id=?", (g["client_id"],))
        if g["client_id"]
        else None
    )
...
async def email_gallery(
    ...
    if not g["published"]:
        raise HTTPException(
            status_code=400, detail="publish the gallery first — the link 404s until then"
        )
    ...
    if email_kind == "final" and g["project_id"]:
        ...
        if p and p["status"] in _PRE_DELIVERED_STATUSES:
            db.run(
                "UPDATE projects SET status='project_closed', "
                "stage_changed_at=datetime('now') WHERE id=?",
                (g["project_id"],),
            )
            log.info(
                "project %s auto-advanced to 'project_closed' via final email (was %s)",
                g["project_id"],
                p["status"],
            )
    return RedirectResponse(f"/admin/galleries/{gallery_id}", status_code=303)
...
async def delete_gallery(gallery_id: int, force: bool = Form(False)):
    ...
    if g["published"] and g["type"] != "drop":
        raise HTTPException(
            status_code=400,
            detail="unpublish first — deleting a live client link is a two-step on purpose",
        )
...
async def admin_add_comment(
    ...
    return RedirectResponse(f"/admin/galleries/{gallery_id}#asset-{asset_id}", status_code=303)
...
async def admin_hide_comment(comment_id: int):
    ...
    return RedirectResponse(
        f"/admin/galleries/{c['gallery_id']}#asset-{c['asset_id']}", status_code=303
    )
...
def _transition_comment(comment_id: int, *, want: str, to: str):
    ...
    c = db.one(
        "SELECT id, asset_id, gallery_id, parent_id, status, deleted_at "
